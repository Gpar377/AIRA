...
class PurpleReport(TypedDict):
    """Security posture report generated by the Purple Agent after a battle."""
    campaign_id: str
    total_rounds: int
    # Attack analysis
    attack_types_attempted: Dict[str, int]    # vuln_type → count
    attack_success_rate: float                # 0.0–1.0
    attack_chain_depth: int                   # longest observed chain
    most_exploited_namespace: str
    most_exploited_resource: str
    # Defense analysis
    defense_types_deployed: Dict[str, int]    # defense_type → count
    defense_coverage_gaps: List[str]          # namespaces/resources never defended
    pre_emptive_defense_rate: float           # fraction of proactive vs reactive
    # Governance
    opa_block_rate: float                     # fraction of actions blocked
    escalation_attempts: int
    kill_switch_triggered: bool
    # Score trajectory
    initial_score: float
    final_score: float
    score_reduction_pct: float
    # LLM-generated insights
    pattern_synthesis: str                    # key attack/defense patterns observed
    blind_spots: str                          # undefended areas and missed opportunities
    recommendations: List[str]               # prioritized remediation actions
...
class ArenaEvent(TypedDict):
    timestamp: str
    round: int
    agent: str               # red | blue | purple | orchestrator | system
    event_type: str          # scan | propose_attack | opa_check | execute | patch | round_end | kill_switch | purple_report
    message: str
...
class ArenaState(TypedDict):
    ...
    # Purple Agent output (populated at end of battle)
    purple_report: Optional[PurpleReport]
