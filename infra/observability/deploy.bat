...
echo 📝 Deploying Loki...
helm upgrade --install loki grafana/loki-stack --namespace observability --set loki.persistence.enabled=true --set loki.persistence.size=5Gi --set promtail.enabled=true --set loki.isDefault=false --wait

echo 🔍 Deploying Jaeger...
helm upgrade --install jaeger jaegertracing/jaeger --namespace observability --set provisionDataStore.cassandra=false --set allInOne.enabled=true --set storage.type=memory --set agent.enabled=false --set collector.enabled=false --set query.enabled=false --wait
